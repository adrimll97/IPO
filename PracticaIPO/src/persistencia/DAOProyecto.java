...
public class DAOProyecto {
	...
	public boolean actualizarProyectoDAO(int idProject, String nombre, Date fechaCreacion, int responsable, String descripcion) {
		...
		try {
			realizado = true;			
			con.createStatement();
			final String sql = "update usuarios where idProyecto = " + idProject + " set nombre = " + nombre + 
					", fechaCreacion = " + fechaCreacion + ", responsable = " + responsable + ", descripcion = " + descripcion;
			pst = con.prepareStatement(sql);
			pst.executeUpdate();
		} catch (Exception e) {
