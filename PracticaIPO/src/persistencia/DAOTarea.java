...
public class DAOTarea {
	...
	public boolean insertarTareaDAO(String nombre, Date fechaInicio, Date fechaFin, String estado, 
		...
		try {
			...
			System.out.println("Error al insertar proyecto en la base de datos."+e.getMessage());
			realizado = false;
		}
		return realizado;
	}
	...
	public boolean actualizarTareaDAO(int idTarea, String nombre, Date fechaInicio, Date fechaFin, String estado, 
		...
		try {
			realizado = true;			
			con.createStatement();
			final String sql = "update usuarios where idTareas = " + idTarea + " set nombre = " + nombre + 
					", fechaIni = " + fechaInicio + ", fechaFin = " + fechaFin + ", estado = " + estado +
					", prioridad = " + prioridad + ", encargado = " + encargado + ", comentarios = " + comentarios;
			pst = con.prepareStatement(sql);
			pst.executeUpdate();
		} catch (Exception e) {
